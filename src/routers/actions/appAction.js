...
import { getConversationTitle, getAiResponse } from "../../api/geminiAi";
...
const userPromptAction = async (formData) => {
    ...
    try {
        conversation = await databases.createDocument(import.meta.env.VITE_APPWRITE_DATABASE_ID, 'conversations', generateID(), {
...
const deleteConversationAction = async (formData) => {
    ...
    try {
        await databases.deleteDocument(import.meta.env.VITE_APPWRITE_DATABASE_ID, 'conversations', conversationId);
